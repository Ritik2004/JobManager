...
const JobScore = ({job, setShowScore}) => {
    ...
    const getScore = async(resume,notes)=>{
         try{
            ...
            alert(`Resume Fit Score: ${data.score}\nFeedback: ${data.feedback}`);   
                setShowScore(false);
         }
          catch(err){
  ...
  return (
    <div className="h-full flex flex-col">
        <form className="flex-1 flex flex-col p-6 bg-white rounded-xl border border-gray-200 relative">
            ...
            <button className=' bg-purple-600 text-white py-3 px-4 w-fit rounded-lg' onClick={(e) => {e.preventDefault(); setShowScore(false);getScore(resume, job.notes)}}>
                Get Score
            </button>
        </form>
    </div>
